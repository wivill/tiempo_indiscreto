...
void tarea01Fun() {
  ...
  if (M == 1023) {
    ...
    Serial.print("\n");
      ...
      if (j == 0) {
        Serial.println("Entrando al modo Automático");
        j++;
        }
        // Definicion de ecuacion u del controlador
        // Lectura de Set Point Sp proveniente de A4
        Sp      =   analogRead(A4);
        ...
        P       =   kp*(B*Sp - Y);
        Serial.print("Parte Proporcional P          ");
        Serial.println(P);
        I       =   Iante + ((kp*ts)/(ti))*(SPante - Yante);
        Serial.print("Parte Integral I             ");
        Serial.println(I);
        //Señal a pin PWM
        U       =   P + I;
